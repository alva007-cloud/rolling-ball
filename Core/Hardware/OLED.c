...
{
  if (mode == COLOR_NORMAL)
  {
    OLED_SendCmd(0xA6); // 正常显示
  }
  if (mode == COLOR_REVERSE)
  {
...
void OLED_ShowChar(uint8_t Line, uint8_t Column, char Char)
{
	...
	for (i = 0; i < 8; i++)
	{
		OLED_Send(OLED_F8x16[Char - ' '][i],1);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 8; i++)
	{
		OLED_Send(OLED_F8x16[Char - ' '][i + 8],1);		//显示下半部分内容
	}
}
...
void OLED_ShowNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	uint8_t i;
	uint8_t numlength;
	numlength = (int)log10(Number)+1;
	for (i = 0; i < Length; i++)
	{
		if(numlength < (Length-i))continue;
...
void OLED_format_Show_string(uint8_t x, uint8_t y,char *format,...)
{
	uint8_t string[256];
	va_list arg;
	va_start(arg,format);
	...
	va_end(arg);
	OLED_PrintString(x, y, string, &font16x16, COLOR_NORMAL);
}
